...
export default async function QuotesPage({ searchParams }: QuotesPageProps) {
  const { status, after, before } = await searchParams
  const statusFilter = status === 'all' || !status ? undefined : status

  const result = await getQuotes(statusFilter, { after, before })
  const { data: quotes, nextCursor, prevCursor } = result

  // Preserve the status filter param in pagination hrefs
  const statusParam = statusFilter ? `&status=${statusFilter}` : ''
  const prevHref = prevCursor ? `/dashboard/quotes?before=${prevCursor}${statusParam}` : null
  const nextHref = nextCursor ? `/dashboard/quotes?after=${nextCursor}${statusParam}`  : null
        ...
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between md:mb-8">
          <div>
            ...
            <p className="mt-0.5 text-sm text-zinc-400">
              {quotes.length > 0
                ? `Showing ${quotes.length}${result.hasNextPage ? '+' : ''}`
                : 'No quotes'}
            </p>
          </div>
          <Link
            ...
            <div className="space-y-2 sm:hidden">
              {quotes.map((quote) => {
                const client = Array.isArray(quote.clients)
                  ? quote.clients[0]
                  : (quote.clients as { first_name: string; last_name: string } | null)
                return (
                  <Link
                    key={quote.id}
